...
/** Parallax starfield + brighter "constellation" points. */
...
/** Hero focal objects: a big distorting-ish crystal + accent ring. */
...
/** Distant planet + its glowing atmosphere, doubling as the key light. */
function Planet() {
  ...
  return (
    <group position={[3.6, 3, -7]}>
      <mesh ref={ref}>
        <sphereGeometry args={[1.7, 64, 64]} />
        <meshStandardMaterial color="#c8d0ff" emissive="#8f9dff" emissiveIntensity={0.6} roughness={0.9} metalness={0.1} />
      </mesh>
      {/* atmosphere halo */}
      <mesh scale={1.28}>
      ...
      <pointLight color="#c9d2ff" intensity={45} distance={34} decay={2} />
    </group>
  );
}
...
function LightRays() {
  ...
  const rays = 7;
  return (
    <group ref={group} position={[3.4, 2.8, -6.5]}>
...
function Rig() {
  ...
  useFrame((_, delta) => {
    ...
    const breathe = Math.sin(t * 0.25) * 0.35; // gentle in/out
    target.current.set(
